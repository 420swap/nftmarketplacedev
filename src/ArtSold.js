...
class ArtSold extends React.Component {
  ...
  async loadDigitalArts(web3) {
      try {
        let ids, soldArray;
        const result = await this.state.contractInstance.methods.findAllArt().call();
        ids = result[0];
        soldArray = result[3]
        let newIds = [];
      ...
      return (
        <div className="App" style={sectionStyle}>
          ...
          <div style={{color:"white"}}className="h1-responsive font-weight-bold text-center my-5">
              View digital Art recently traded on the Avalanche Blockchain using 420Swap
            </div>
                  ...
                  {this.state.columns.map((col, j) =>
                    <div  style={{marginTop:"50px"}} className="col-lg-4 col-md-12 mb-lg-0 mb-0" key={j}>
                        { i*3+j < this.state.total &&
                            ...
                            <p style={{whiteSpace: 'nowrap',overflow: 'hidden',textOverflow:'ellipsis'}}><span className="font-weight-bold">@{this.state.author[i*3+j]}</span></p>
